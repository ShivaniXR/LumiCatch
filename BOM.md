...
[`wiring-diagram.png`](wiring-diagram.png). For which hole each jumper actually
goes into, see the breadboard view,
[`breadboard-view.png`](breadboard-view.png).**
...
server, the HTTP dashboard, both AI models and the shared shoal simulation are
written against the standard library alone, including a hand-rolled RFC 6455
implementation. App Lab
...
| Item | Source | Licence |
|---|---|---|
| `simple_jellyfish.glb` | ['Simple Jellyfish'](https://sketchfab.com/3d-models/simple-jellyfish-f77876d8297846eeb23c4ad82dbebb97) by RickStikkelorum, on Sketchfab | [CC BY 4.0](https://creativecommons.org/licenses/by/4.0/) |
| `SuperRareJelly/Spotted-Jelly.gltf` | ['Simple Spotted Jellyfish (Baked Animation)'](https://sketchfab.com/3d-models/simple-spotted-jellyfish-baked-animation-d5006697ad3c4bc1ac814110cde19af2) by n-, on Sketchfab | [CC BY 4.0](https://creativecommons.org/licenses/by/4.0/) |
...
| `ChimeRare.wav` | Generated by [`make-audio.py`](make-audio.py) | Ours |
| `ChimeSuper.wav` | Generated by [`make-audio.py`](make-audio.py) | Ours |
| `MissWhiff.wav` | Generated by [`make-audio.py`](make-audio.py) | Ours |
| `CountBeep.wav` | Generated by [`make-audio.py`](make-audio.py) | Ours |
| `CountGoSfx.wav` | Generated by [`make-audio.py`](make-audio.py) | Ours |
| `GameOver.wav` | Generated by [`make-audio.py`](make-audio.py) | Ours |
| Start screen and end screen artwork | Made for this project | Ours |

All eight sounds are synthesised from scratch by a standard-library Python
script, so they are ours to publish and the ambient bed loops seamlessly. Every
partial in it completes a whole number of cycles inside the 8 second buffer,
which is the only way to make a loop point genuinely inaudible.

Every sound is also band limited on purpose: nothing meaningful sits below
110 Hz, because Spectacles' speakers cannot move air down there and distort
trying, which is heard as a rattle rather than as bass. Measured across the six
newer files, energy below 110 Hz is between 0.00 and 0.03 per cent.

### Third-party model attribution

Both jellyfish models are **CC BY 4.0**, which is the permissive Creative
Commons licence: commercial use is allowed and derivatives are allowed. The
only condition is attribution, given in full here and in
[`README.md`](README.md):

> **'Simple Jellyfish'** by **RickStikkelorum**, licensed under
...
> **'Simple Spotted Jellyfish (Baked Animation)'** by **n-**, licensed under
> [CC BY 4.0](https://creativecommons.org/licenses/by/4.0/). Source:
> <https://sketchfab.com/3d-models/simple-spotted-jellyfish-baked-animation-d5006697ad3c4bc1ac814110cde19af2>

**Changes made:** neither model file has been altered. Both are used as
imported. At runtime the Lens replaces each model's own PBR material with an
unlit, self-coloured, two-sided translucent material so the creatures read as
bioluminescent on an additive display, and scales them and corrects the
imported animation clip length. No geometry, rigging or animation data was
edited.

CC BY 4.0 is compatible with this repository's MIT licence: the MIT licence
covers the code, and the two models remain under CC BY 4.0 with the attribution
above. Do not remove these credits.
...
| Swing detection threshold | 3.0 g |
| Sneak / lunge split | 4.0 g |
| MCU peak capture window | 80 ms |
| Linux poll interval | 20 ms, 50 Hz |
